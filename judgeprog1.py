...
def check_and_modify_Extract(t1,t2):
    ...
    if not newfuncnode:
        return False
    calltree=checkFuncCall(newfuncnode.name,t2)
    if not calltree: #新たに定義された関数の定義を発見
        return False
    ...
    exprnode=calltree.parent#呼び出しを示す文の木の先頭となるexprノードを取得
